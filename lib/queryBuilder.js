...
QueryBuilder.prototype.delete = function() {
  ...
  this.buffer[Index.FROM] = "FROM "+this.escapedTableName;
...
QueryBuilder.prototype.select = function(clause) {
  ...
  if (isArray) {
    ...
    if (fields.length === 0)
      ...
      this.buffer[Index.SELECT] = "SELECT "+fields.join(',');
  }
...
QueryBuilder.prototype.update = function() {
  ...
  this.buffer[Index.UPDATE] = "UPDATE "+this.escapedTableName;
...
function validFieldsInArray(that, arr) {
  var i, len = arr.length, field, fields = [];

  for (i = 0; i < len; i++) {
    field = arr[i];
    if (that.schema.columns.indexOf(field) > -1)
      fields.push(field);
  }
  return fields;
}
