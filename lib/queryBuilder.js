...
QueryBuilder.prototype.delete = function() {
  this.reset();
  this._type = QueryType.DELETE;
  this.buffer[Index.DELETE] = "DELETE ";
  this.buffer[Index.FROM] = "FROM "+this._schema.escapedTableName;
...
QueryBuilder.prototype.update = function() {
  this.reset();
  this._type = QueryType.UPDATE;

  this.buffer[Index.UPDATE] = "UPDATE "+this._schema.escapedTableName;
...
QueryBuilder.prototype.set = function(any, locals) {
  if (this._type !== QueryType.UPDATE) this.update();
  ...
  if (_.isString(any)) {
    ...
    if (_.isEmpty(pred)) {
      if (this._strict)
        ...
        this.buffer[Index.WHERE] = "WHERE 1=0";
    }
    else {
...
QueryBuilder.prototype.toSql = function() {
  if (this._strict) {
    if (this._type === QueryType.UPDATE && !this.buffer[Index.WHERE])
      throw new Error("STRICT: WHERE clause missing for UPDATE operation");
    if (this._type === QueryType.DELETE && !this.buffer[Index.WHERE])
 ...
 * Populate a relation.
 */
QueryBuilder.prototype.populate = function(property, lambda) {
