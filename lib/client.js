 ...
 */
Client.prototype.connect = function(configureFn) {
  var config = this.config;
  var client = mysql.createConnectionSync();
  this.client = client;
  client.initSync();

  // save configuration function in case connect is called many times
  if (!this.configureFn) this.configureFn = configureFn;

  if (this.configureFn) {
    this.configureFn(this.client);
  }
  else {
    if (config.charset) {
