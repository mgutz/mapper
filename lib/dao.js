 ...
 *
 */
Dao.prototype.execSeries = function() {
  ...
  async.forEachSeries(queryArgs, function(statementArgs, cb) {
    that.sql.apply(that, statementArgs).all(function(err, rows) {
...
Dao.prototype.execParallel = function() {
  // remove callback, rest are queries
  var args = __slice.call(arguments, 0);
  var cb = args[args.length - 1];
  var i, arg, L;
  ...
  async.forEach(queryArgs, function(statementArgs, cb) {
    that.sql.apply(that, statementArgs).all(function(err, rows) {
      if (err) return cb(err);
      results.push(rows);
      cb();
