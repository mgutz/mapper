 ...
 * Populate a relation.
 */
Relation.prototype.populate = function(property, lambda) {
...
function fetchPopulations(that, rows, cb) {
  ...
  function populate(item, cb) {
    ...
    function fetch(err, inRelation) {
      ...
      // `relation` is set abbove if a callback was provided for `populate`
      var relation = buildRelation(that, relationItem, rows, inRelation);

      // if (relationItem.name == 'post') {
      ...
      relation.all(function(err, relationRows) {
        ...
        //   console.log(that.schema.relations);
        //   console.log(dao.peekSql());
        //   process.exit(0);
        // }
